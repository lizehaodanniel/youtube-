...
「每日热点看台」—— 一个面向多 YouTube 频道的 **每日/每月选题推荐报告 + 置顶成片包** 自动生成器。输入真实搜索下拉词与公开数据，产出一份可发布的深色响应式 HTML 报告（每频道 5 板块选题推荐），并在报告最顶部钉两个**完整成片包**（含 3 个 Hook 备选、句子级英中分栏脚本、图片提示词、A/B 封面、简介/标签/章节）。
...
## 核心纪律：词数 ≈ 时长 × 2.4（防逻辑漏洞）
一个 75 秒的 beat 必须 ~180 词，绝不能用一句话敷衍。每个 beat 低于目标 80% 词数即打回重写。详见 `references/methodology.md` §4。
